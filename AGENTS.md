...
- `@apostoli/use-external-state`: core hook + browser storage adapters (Zod-only)
- `@apostoli/use-external-state/next`: query-parameter adapter for the Next.js App Router
...
- `examples/next-query-params`: Next.js App Router example depending on `@apostoli/use-external-state/next`.
